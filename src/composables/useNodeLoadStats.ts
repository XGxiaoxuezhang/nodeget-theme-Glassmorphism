...
function warnZeroSamplesIfNeeded(entry: SharedLoadRecordsEntry, uuid: string, cacheKey: string, online: boolean, maxCount?: number): void {
  ...
  console.warn(`[NodeGet Glassmorphism] capped shared load history returned zero samples for online node ${uuid}; maxCount=${LOAD_RECORD_MAX_COUNT}. This may indicate cap starvation or missing backend history.`)
}
